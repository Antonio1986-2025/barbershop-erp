...
export async function fetchCurrentCash(unitId: string): Promise<CashSummary | null> {
  const res = await fetch(`${API_BASE}/api/cash/current?unitId=${unitId}`, { headers: headers() });
...
export async function closeCash(id: string, data?: {
  ...
  const res = await fetch(`${API_BASE}/api/cash/${id}/close`, {
    method: 'POST', headers: headers(), body: JSON.stringify(data ?? {}),
  });
  if (!res.ok) throw new Error(await res.text());
  return res.json();
...
export async function reopenCash(id: string): Promise<CashRegister> {
  const res = await fetch(`${API_BASE}/api/cash/${id}/reopen`, {
...
export async function supplyCash(id: string, data: {
  ...
  const res = await fetch(`${API_BASE}/api/cash/${id}/supply`, {
    method: 'POST', headers: headers(), body: JSON.stringify(data),
  });
  if (!res.ok) throw new Error(await res.text());
  return res.json();
...
export async function withdrawCash(id: string, data: {
  ...
  const res = await fetch(`${API_BASE}/api/cash/${id}/withdraw`, {
    method: 'POST', headers: headers(), body: JSON.stringify(data),
  });
  if (!res.ok) throw new Error(await res.text());
  return res.json();
...
export async function fetchCashSummary(id: string): Promise<CashSummary> {
  const res = await fetch(`${API_BASE}/api/cash/${id}/summary`, { headers: headers() });
  if (!res.ok) throw new Error(await res.text());
  return res.json();
}

export async function fetchCashHistory(unitId?: string): Promise<CashHistoryItem[]> {
  const url = new URL(`${API_BASE}/api/cash/history`);
